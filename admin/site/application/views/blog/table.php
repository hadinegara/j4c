		...
		<?php if(is_array($post) && count($post)>0): ?>
			...
			<?php foreach($post as $row): ?>
				...
				<tr class="post-item">
					...
					<td>
						<a href="<?php echo base_url('blog/edit/'. $row['id']); ?>" class="btn btn-small">Edit</a>
						<a href="#" class="btn btn-small btn-warning btn-archive" data-id="<?php echo $row['id']; ?>">Archive</a>
					</td>
				</tr>
			<?php endforeach; ?>
		<?php else: ?>
		...
		me.bind('click', function(e){
			...
			if(confirm('Are you sure you want to archive this post?')){
				$.ajax({
					...
					data: {id: me.attr("data-id"), status: 'archive'},
					dataType: "json",
					success: function(r){
						if(r.success == true)
						{
